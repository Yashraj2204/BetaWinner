...
function renderInsight(text) {
  ...
  return text.split("\n").map((line) => {
    ...
    const bolded = t
      ...
      .map((part) =>
        part.startsWith("**") ? (
          <strong key={part} className="text-[#1A2E20] font-semibold">
            {part.slice(2, -2)}
          </strong>
        ) : (
          part
    ...
    if (t.startsWith("###"))
      return (
        <h3
          key={t}
          className="font-heading font-bold text-[#1A2E20] text-sm uppercase tracking-[0.15em] mt-4 first:mt-0 mb-2"
        >
          {bolded}
        </h3>
      );
    if (t.startsWith("- "))
      return (
        <div key={t} className="flex gap-2.5 mb-2 text-sm text-[#3d4a42] leading-relaxed">
          <span
            className="mt-1.5 w-1.5 h-1.5 rounded-full bg-[#E06D53] shrink-0"
            aria-hidden="true"
          />
          <span>{bolded}</span>
        </div>
    ...
    return (
      <p key={t} className="text-sm text-[#3d4a42] leading-relaxed mb-2">
        {bolded}
      </p>
    );
  });
      ...
      <div className="relative p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2.5">
            <span
              className="w-8 h-8 rounded-lg bg-[#8BA888]/20 flex items-center justify-center"
              aria-hidden="true"
            >
        ...
        >
          {shown ? (
            ...
            <p className="text-sm text-[#4A5A50] py-8 text-center">
              Click <strong>Generate</strong> to get a personalised assessment and tailored
              reduction tips.
            </p>
          )}
        </div>
